...
volatile int cor_atual;
volatile int cor_anterior;
...
void startLeitura() {
  ...
  while(1)
  {
    ...
    if(estado_atual == CAPTURA && cor_atual == BRANCO) {
      estado_atual = REG_B;
      //chamar funcao pra gravar texto (em outra thread)
      cog = cog_run(gravarValor,128);
...
inline void gravarValor() {
  ...
  fwrite(estado, 1, 2, fp);      // Add contents to the file
